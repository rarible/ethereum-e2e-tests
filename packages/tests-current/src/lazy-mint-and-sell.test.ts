...
describe("lazy mint test", function () {
	...
	const {
		...
		wallet1,
		wallet2,
	} = initProviders({pk2: "ded057615d97f0f1c751ea2795bc4b03bbf44844c13ab4f5e6fd976506c276b9"})
	...
	/*
	TODO: this test does not work because SDK of the old version had a bug: lazy item 1155 was created as 721.
	Update the e2e tests to use the new SDK and implement this test.
	...
	test("should lazy mint and sell ERC1155 token", async () => {
		...
		await verifyNewOwner(sdk2, mintResponse.itemId, toAddress(wallet2.getAddressString()))
		await verifyEthBalance(web32, toAddress(wallet2.getAddressString()), toBn(balanceBefore)
			.minus(100 * 1000000).toString())
	}, 50000)*/
