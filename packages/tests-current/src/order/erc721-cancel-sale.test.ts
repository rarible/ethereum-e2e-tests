...
import { OrderActivityFilterByItemTypes, OrderStatus } from "@rarible/ethereum-api-client"
...
describe("cancel order test", function () {
	...
	test("cancel sell erc721 <-> erc-20", async () => {
		...
		await verifyOrderActivities(sdk1, conf.testErc721, "1", new Map([
				...
				[OrderActivityFilterByItemTypes.CANCEL_LIST, 1]
		]))
	})
})
